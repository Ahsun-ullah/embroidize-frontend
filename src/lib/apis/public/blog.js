...
export async function getResources(searchQuery, currentPage, perPageData) {
  try {
    const headers = new Headers();
    headers.set('Authorization', 'Bearer some-static-token');
    ...
    const apiUrl = `${process.env.NEXT_PUBLIC_BASE_API_URL_PROD}/public/resource?${queryParams.toString()}`;
