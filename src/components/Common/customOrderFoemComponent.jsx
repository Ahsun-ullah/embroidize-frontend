...
const schema = z.object({
  ...
  file: z
    .custom((file) => file instanceof File && file.size > 0, 'Please upload a design file.')
    .refine((file) => file && file.size <= bytesFromMB(MAX_FILE_MB), `Max file size ${MAX_FILE_MB}MB`),
  // extra fields
  fileFormat: z.enum(['DST', 'PES', 'EMB', 'PDF', 'PNG'], { required_error: 'Choose a format' }),
  turnaround: z.enum(['Standard (4–8h)', 'Rush (2–4h)', 'Next Day'], { required_error: 'Choose turnaround' }),
  complexity: z.enum(['Simple', 'Medium', 'Complex'], { required_error: 'Choose complexity' }),
  sizeWidth: z.coerce.number().min(0.1, 'Enter width').max(100, 'Width too large'),
  sizeHeight: z.coerce.number().min(0.1, 'Enter height').max(100, 'Height too large'),
    ...
    <section className="mx-auto mb-14 max-w-xl">
      ...
      >
        ...
        <div className="text-zinc-700">
          ...
          <p className="text-sm font-semibold">Drag & drop your design files here</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="grid gap-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-1">
            ...
            <input type="text" {...register('name')} required className={baseField} />
            {errors.name && <span className={errorText}>{errors.name.message}</span>}
          </label>
          <label className="grid gap-1">
            <span className={baseLabel}>Email</span>
            <input type="email" {...register('email')} required className={baseField} />
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-1">
            ...
            <select {...register('fileFormat')} className={baseField} defaultValue="">
            ...
            {errors.fileFormat && <span className={errorText}>{errors.fileFormat.message}</span>}
          </label>

          <label className="grid gap-1">
            <span className={baseLabel}>Turnaround</span>
            <select {...register('turnaround')} className={baseField} defaultValue="">
              <option value="" disabled>Choose an option</option>
              <option value="Standard (4–8h)">Standard (4–8h)</option>
              <option value="Rush (2–4h)">Rush (2–4h)</option>
              <option value="Next Day">Next Day</option>
            </select>
            {errors.turnaround && <span className={errorText}>{errors.turnaround.message}</span>}
          </label>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <label className="grid gap-1">
            ...
            <div className="flex gap-2">
              <input type="number" step="0.1" placeholder="Width" {...register('sizeWidth')} className={baseField} />
              <input type="number" step="0.1" placeholder="Height" {...register('sizeHeight')} className={baseField} />
          ...
          <label className="grid gap-1">
            ...
            <select {...register('complexity')} className={baseField} defaultValue="">
              <option value="" disabled>Choose an option</option>
              <option value="Simple">Simple</option>
              <option value="Medium">Medium</option>
              <option value="Complex">Complex</option>
            </select>
            {errors.complexity && <span className={errorText}>{errors.complexity.message}</span>}
          </label>
        </div>
        ...
        <label className="grid gap-1">
          ...
          <textarea rows={4} {...register('details')} required className={baseField} />
          {errors.details && <span className={errorText}>{errors.details.message}</span>}
        </label>
