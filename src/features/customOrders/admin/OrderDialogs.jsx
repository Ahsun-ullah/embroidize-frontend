...
// Every dialog on the admin custom-orders screen, plus the handlers that drive
// them. Split out of CustomOrdersTableWrapper (which was a single 3.1k-line
// client component) and pulled in with next/dynamic, so none of this — twelve
// modals, the HeroUI modal/select/textarea surface, the message thread and the
// receipt renderer — ships in the route's first JS chunk. It mounts the first
// time the admin picks a row action.
//
// The table owns no dialog state: it passes a { kind, order, url, nonce }
// request and this component dispatches it to the matching handler.
...
export default function OrderDialogs({ request, onHandled }) {
  ...
  // The row from the list endpoint is deliberately thin — the free-text brief,
  // the payments ledger and the IP block are stripped there so 20 rows stay
  // small. This modal is the only place that needs them, so it fetches the full
  // document alongside the activity log and renders the row meanwhile.
  const handleViewDetails = async (order) => {
    setViewOrder(order);
    setActivityList([]);
    setLoadingActivity(true);
    onViewModalOpen();
    const [full, activity] = await Promise.allSettled([
      fetch(`${apiBase()}/admin/orders/custom/${order._id}`, {
        headers: adminHeaders(false),
      }).then((r) => (r.ok ? r.json() : null)),
      fetch(`${apiBase()}/admin/orders/custom/${order._id}/activity`, {
        headers: adminHeaders(false),
      }).then((r) => (r.ok ? r.json() : null)),
    ]);
    const fullOrder = full.status === 'fulfilled' ? full.value?.data : null;
    if (fullOrder?._id) {
      // Keep the list-only extras (repeat-customer counts, unread badge) that
      // the detail endpoint does not compute.
      setViewOrder((prev) => ({ ...prev, ...fullOrder }));
    }
    if (activity.status === 'fulfilled') {
      setActivityList(activity.value?.data?.activity || []);
    }
  ...
  // Row actions arrive as a request object from the table. Every action —
  // including the three that open no modal (download, receipt, resend link) —
  // is dispatched here so the table never has to know about any of them.
  useEffect(() => {
    if (!request) return;
    const { kind, order, url } = request;
    switch (kind) {
      case 'view':
        handleViewDetails(order);
        break;
      case 'messages':
        handleOpenMessages(order);
        break;
      case 'status':
      ...
      case 'payment-request':
        handlePayRequest(order);
        break;
      case 'record-payment':
        handleRecordPayment(order);
        break;
      case 'offer-payment':
        handleOfferPayment(order);
        break;
      case 'record-refund':
        handleRecordRefund(order);
        break;
      case 'deliver':
        handleDeliver(order);
        break;
      case 'download-delivery':
        downloadDelivery(order);
        break;
      case 'view-invoice':
